...
class QuanLyNguoiDung extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.reRender !== this.state.reRender ||
      prevState.themXoa !== this.state.themXoa
    ) {
      this.props.layDanhSachNguoiDung();
    }
  ...
  handleSubmit = () => {
    ...
    const index = this.props.danhSachNguoiDung.findIndex(
      (nd) => nd.taiKhoan === nguoiDung.taiKhoan
    );
    if (index === -1) {
...
const mapDispatchToProps = (dispatch) => ({
  layDanhSachNguoiDung: (danhSachNguoiDung) =>
    dispatch(LayDanhSachNguoiDung(danhSachNguoiDung)),
