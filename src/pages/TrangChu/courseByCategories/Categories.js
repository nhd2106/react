    ...
    <div className="container mt-5 mb-5 " data-aos="fade-up">
      <div>
        ...
        <p>
          ...
          neque sint eveniet tempore sapiente.
        </p>
      </div>

      <div className="row">
        {danhMucKhoaHoc.map((danhMuc) => (
          <div
          key={danhMuc.maDanhMuc}
            className={`col-xs-12  col-sm-6 col-md-4 col-lg-4 ${myStyle.categories_outer}`}
          >
            {" "}
            <div 
              className={`text-center ${myStyle.categories_inner}`}
              type="button"
              onClick={() =>
                props[0].history.push(
                  `/khoa-hoc-theo-danh-muc/${danhMuc.maDanhMuc}`
                )
              }
            >
              {danhMuc.tenDanhMuc}
      ...
      <Slider className="mt-5 " {...settings}>
        {danhSachKhoaHoc.map((item) => (
          <div key={item}>
            <div
              className="card text-right text-dark bg-light "
              type="button"
              onClick={() =>
